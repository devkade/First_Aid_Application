...
public class NcommentB extends AppCompatActivity {
    ...
    numQuiz quizNB =new numQuiz();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        List<List<String>> Quiz=new ArrayList<List<String>>();
        for(int i=0; i<quizB.getoxB().size(); i++){
            Quiz.add(quizB.getoxB().get(i));
        }

        for(int i = 0; i< quizNB.getnumA().size(); i++){
            Quiz.add(quizNB.getnumA().get(i));
        }
